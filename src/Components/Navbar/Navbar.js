...
const Navbar = () => {
  return (
    <div className="w-full py-6 px-16 fixed z-20 flex justify-between items-center">
      <a href="/">
        <svg version="1.1" x="0px" y="0px" viewBox="0 0 400 50" className="w-[230px]">
        ...
        <ul>
            {
                NavbarItems.map((item, index) => (
                    <NavLink key={index} to={item.link} className="text-white text-[14px] mr-4 font-bold uppercase hover:border-b-2"> {item.text} </NavLink>
                ))
            }
        ...
        <div className="text-white flex items-center list-none space-x-4 font-semibold">
          ...
          <li><BiMenuAltRight className="text-xl" /></li>
        </div>
    </div>
  );
};
